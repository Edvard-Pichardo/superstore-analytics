/*
Archivo      : 06_product_analysis
Proyecto     : superstore-analytics
Autor        : Cristian Eduardo Pichardo Rico
Descripción  : Análisis del desempeño comercial de los productos, considerando ventas, beneficio,
               margen, participación, pérdidas y cobertura de información.
*/
...
-- Posición dentro de su propia subcategoría.
-- product_key se utiliza como identificador principal porque algunos product_id originales fueron reutilizados.
...
-- Las ventas promedio de todos los productos.
-- El margen comparable promedio de todos los productos.

-- Esto permite detectar productos con un volumen comercial elevado pero una rentabilidad relativamente baja.
WITH referencias AS(
    SELECT
        AVG(total_sales)
...
-- Ordenamos los productos por ventas conocidas y calculamos su participación individual y acumulada dentro del negocio.
-- Clasificación ABC:
...
-- Cantidad mínima de productos necesaria para alcanzar distintos niveles de concentración de ventas.
...
-- Validamos que la participación de todos los productos represente el 100% de las ventas conocidas.
SELECT
    ROUND(
        SUM(
            100.0
...
-- Identificamos los códigos originales asociados con más de un producto dentro del modelo normalizado.
...
-- Impacto comercial de los productos cuyos códigos originales fueron reutilizados.
WITH codigos_reutilizados AS
(
    SELECT
        source_product_id
...
-- product_key mantiene separados correctamente los productos cuyos identificadores originales fueron reutilizados.
