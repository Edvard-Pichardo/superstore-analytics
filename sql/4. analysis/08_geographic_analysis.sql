/*
Archivo      : 08_geographic_analysis
Proyecto     : superstore-analytics
Autor        : Cristian Eduardo Pichardo Rico
Descripción  : Análisis geográfico del desempeño comercial, considerando ventas, beneficio, clientes,
...
-- Comparamos las ciudades según ventas, beneficio, volumen de pedidos y margen comparable.
...
-- Validamos que todas las ciudades del modelo analítico estén representadas en la vista geográfica.
SELECT
...
-- Comparamos cada ciudad contra las ventas y el margen promedio de todas las ciudades del negocio.
WITH referencias AS(
    SELECT
        AVG(total_sales)
...
-- Comparamos las regiones según volumen comercial, rentabilidad, clientes, logística y cobertura de datos.
...
-- Validamos que las cuatro regiones representen el 100% de los pedidos y ventas.
...
-- Analizamos ventas, beneficio, clientes, pedidos, rentabilidad y cobertura para cada estado.
...
-- Validamos que la agregación por estado conserve todos los pedidos, ventas y beneficios del negocio.
WITH estados AS(
    SELECT
        region,
        country,
...
-- Comparamos los países según ventas, beneficio, clientes, pedidos, rentabilidad y logística.
...
-- Ordenamos las ciudades según sus ventas conocidas y calculamos su participación acumulada sobre el negocio.
...
-- Cantidad de ciudades necesarias para alcanzar distintos niveles de concentración de ventas.
WITH ventas_ciudades AS(
    SELECT
        vgp.country,
        vgp.state,
...
-- Validamos que la participación acumulada de ventas represente el 100% de las ventas conocidas.
...
-- Analizamos la evolución de pedidos, clientes, ventas, beneficio y margen comparable para cada región y año.
...
-- Detectamos años en los que una región aumentó sus ventas mientras su margen comparable se deterioró.
WITH rendimiento_anual AS(
    SELECT
        vos.region,
        YEAR(vos.order_date) AS order_year,
...
-- Analizamos únicamente las ciudades cuyo beneficio conocido acumulado es negativo y medimos qué proporción
...
-- Cantidad de ciudades necesarias para alcanzar el 50%, 80% y 95% de las pérdidas conocidas.
WITH ciudades_con_perdidas AS(
    SELECT
        country,
        state,
...
-- Detectamos ciudades con ventas superiores al promedio pero con beneficio conocido acumulado negativo.
WITH referencia AS(
    SELECT
        AVG(total_sales)
            AS average_city_sales
...
-- Comprobamos que país, región, estado y ciudad reproduzcan los totales globales del negocio.
...
-- Verificamos que ningún pedido tenga información geográfica incompleta.
SELECT
    SUM(
        CASE
...
-- Cada order_key debe aparecer una sola vez en la vista resumida y tener una única ubicación.
SELECT
    COUNT(*) AS total_orders,
