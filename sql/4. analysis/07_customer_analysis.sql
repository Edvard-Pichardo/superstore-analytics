/*
Archivo      : 07_customer_analysis
Proyecto     : superstore-analytics
Autor        : Cristian Eduardo Pichardo Rico
Descripción  : Análisis del comportamiento y valor de los clientes, considerando ventas, frecuencia,
...
-- Beneficio conocido.
-- Cantidad de pedidos.
-- Valor promedio de pedido completo.
-- Participación sobre las ventas del negocio.
...
-- Valor comercial medido por sus ventas acumuladas.
-- Frecuencia medida por la cantidad de pedidos.

-- NTILE(2) divide los clientes en dos grupos de tamaño aproximadamente igual para cada dimensión.
...
puntuaciones_rfm AS(
    SELECT
        mr.*,
        -- Una menor recencia representa un cliente que compró más recientemente.
        -- Ordenamos de mayor a menor para que los clientes más recientes reciban las puntuaciones más altas.
        NTILE(4) OVER
        (
...
segmentacion_rfm AS(
    SELECT
        pr.*,
        -- Sumamos las tres dimensiones para obtener una puntuación general entre 3 y 12.
        pr.recency_score
        +
        pr.frequency_score
        +
        ...
        -- Clasificamos al cliente considerando de forma conjunta su recencia, frecuencia y valor.
        CASE
...
-- Verificamos que todos los clientes con ventas conocidas hayan recibido una clasificación RFM.
SELECT
...
-- Comparamos las ventas y el margen de cada cliente contra los valores promedio de toda la cartera.
-- Esto permite detectar clientes con ventas superiores al promedio pero con una rentabilidad relativamente baja.
WITH referencias AS(
    SELECT
        AVG(total_sales)
            AS average_customer_sales,
...
-- Clientes con ventas superiores al promedio y margen inferior al promedio de la cartera.
WITH referencias AS(
    SELECT
        AVG(total_sales)
            AS average_customer_sales,
...
-- Clientes con mayor proporción de pedidos completos cuyo beneficio total fue negativo.
SELECT
    customer_id,
    customer_name,
    segment,
...
-- Analizamos cuántos pedidos realiza cada cliente y durante cuántos meses y años diferentes permanece activo.
...
-- Agrupamos a los clientes según el mes de su primera compra y medimos cuántos vuelven a realizar pedidos durante
...
actividad_mensual AS(
    -- Un cliente aparece como máximo una vez por mes, aunque haya realizado varios pedidos.
    SELECT DISTINCT
        vos.customer_id,
        CAST(
            DATE_FORMAT(
...
-- Validamos que cada cliente pertenezca exactamente a una cohorte.
WITH primera_compra AS(
    SELECT
        customer_id,
        CAST(
...
-- Analizamos la composición y el valor de la cartera de clientes dentro de cada segmento.
