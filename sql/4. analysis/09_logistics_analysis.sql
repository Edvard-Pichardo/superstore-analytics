/*
Archivo      : 09_logistics_analysis
Proyecto     : superstore-analytics
Autor        : Cristian Eduardo Pichardo Rico
Descripción  : Análisis del desempeño logístico del negocio, considerando modos de envío, tiempos de entrega,
...
-- Comparamos los modos de envío según utilización, tiempos de entrega, ventas, beneficio y rentabilidad.
-- Los pedidos cuyo modo de envío no pudo recuperarse se conservan explícitamente como 'Unknown'.
...
-- Validamos que los modos de envío conserven todos los pedidos del modelo.
...
-- Clasificamos los pedidos según los días transcurridos entre la fecha del pedido y la fecha de envío.
...
-- Validamos que la clasificación temporal conserve todos los pedidos exactamente una vez.
WITH clasificacion_envios AS(
    SELECT
        order_key,
        CASE
...
-- Cruzamos cada modalidad con los intervalos reales de duración logística observados en los pedidos.
WITH clasificacion_envios AS(
    SELECT
        vos.order_key,
        vos.customer_id,
...
-- Distribución porcentual de tiempos dentro de cada modo de envío.
WITH base_envios AS(
    SELECT
        COALESCE(
            ship_mode,
...
-- Comprobamos la distribución temporal observada de los pedidos clasificados como Same Day.
SELECT
    shipping_days,
    COUNT(*) AS total_orders,
    ROUND(
...
-- Validamos que los intervalos temporales representen el 100% de los pedidos dentro de cada modo de envío.
...
-- Comparamos los tiempos de envío de cada región contra el comportamiento logístico global del negocio.
...
-- Regiones cuyo tiempo promedio de envío supera el promedio global del negocio.
WITH referencia_global AS(
    SELECT
        AVG(shipping_days)
            AS global_average_shipping_days
...
-- Distribución exacta de días de envío dentro de cada región.
SELECT
    region,
    shipping_days,
    COUNT(*) AS total_orders,
...
-- Validamos que la agregación regional conserve todos los pedidos del modelo.
...
-- Comparamos los países según velocidad de envío, variabilidad, frecuencia de envíos largos y uso
...
-- Países cuyo tiempo promedio de envío supera el promedio global del negocio.
WITH referencia_global AS(
    SELECT
        AVG(shipping_days)
            AS global_average_shipping_days
...
-- Distribución exacta de días de envío dentro de cada país.
SELECT
    country,
    shipping_days,
    COUNT(*) AS total_orders,
...
-- Distribución de modos de envío dentro de cada país.
SELECT
    country,
    COALESCE(
        ship_mode,
...
-- Validamos que la agregación por país conserve todos los pedidos del modelo.
...
-- Analizamos cómo cambian los tiempos de envío, su variabilidad y la distribución de pedidos
...
SELECT
    ...
    -- Un valor negativo representa una reducción del tiempo promedio de envío.
    ROUND(
        average_shipping_days
        -
...
-- Clasificamos la evolución logística de cada año respecto al año inmediatamente anterior.
WITH rendimiento_anual AS(
    SELECT
        YEAR(order_date)
            AS order_year,
...
-- Evolución anual de la utilización de cada modo de envío.
SELECT
    YEAR(order_date)
        AS order_year,
    COALESCE(
...
-- Validamos que la agregación anual conserve todos los pedidos del modelo.
...
-- Analizamos el comportamiento logístico de cada mes observado en el periodo completo del dataset.
...
-- Analizamos la estacionalidad logística agrupando el mismo mes de distintos años.
...
-- Validamos la cantidad de meses calendario y combinaciones año-mes observadas.
SELECT
    COUNT(
...
-- Comparamos los intervalos logísticos según valor de pedido, ventas, beneficio, margen y frecuencia de pérdidas.
-- Este análisis identifica asociaciones entre las variables; no demuestra causalidad entre tiempo de envío y rentabilidad.
...
-- Analizamos ventas y rentabilidad para cada duración exacta de envío observada.
SELECT
    shipping_days,
    COUNT(*) AS total_orders,
    ROUND(
...
-- Comparamos pedidos de hasta dos días contra pedidos con tiempos de ocho días o más.
...
-- Validamos que todos los pedidos pertenezcan exactamente a un intervalo logístico.
WITH clasificacion AS(
    SELECT
        order_key,
        CASE
...
-- Comparamos el desempeño de una misma modalidad entre las distintas regiones del negocio.
...
-- Detectamos combinaciones región-modalidad cuyo tiempo promedio supera al promedio global de esa modalidad.
WITH referencia_modalidad AS(
    SELECT
        COALESCE(
            ship_mode,
...
-- Comparamos el tiempo de una misma modalidad entre los distintos países.
WITH referencia_modalidad AS(
    SELECT
        COALESCE(
            ship_mode,
...
-- Matriz de tiempo promedio por región y modo de envío.
SELECT
    region,
    ROUND(
        AVG(
...
-- Validamos que la combinación región-modalidad conserve todos los pedidos del modelo.
...
-- Comprobamos que las principales dimensiones utilizadas durante el análisis reproduzcan los totales del negocio.
WITH referencia_global AS(
    SELECT
        COUNT(*) AS total_orders,
...
-- Validamos la coherencia entre las fechas y el tiempo de envío calculado.
SELECT
    COUNT(*) AS total_orders,
    SUM(
        CASE
...
-- Verificamos los modos de envío conocidos y contabilizamos los valores desconocidos.
...
-- Validamos el periodo temporal utilizado durante todo el análisis logístico.
SELECT
    MIN(order_date)
        AS first_order_date,
    MAX(order_date)
