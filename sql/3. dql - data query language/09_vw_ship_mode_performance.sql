...
Descripción  : Vista de desempeño por modo de envío:
               Resume el desempeño comercial y logístico de cada modalidad de envío.
*/

USE superstore_analytics;

-- Cada fila representa un modo de envío. Los pedidos cuyo ship_mode es NULL se agrupan bajo la etiqueta 'Unknown'
...
SELECT
    ...
    -- Beneficio promedio de pedidos con información completa de profit.
    AVG(
        CASE
    ...
    -- Margen calculado únicamente con pedidos cuyas ventas y beneficios están completos.
    SUM(
        CASE
            WHEN vos.unknown_sales_lines = 0
...
-- Comparamos los modos de envío presentes en los pedidos con las filas creadas en la vista.
...
-- Verificamos que cada modalidad presente en los pedidos tenga correspondencia en la vista.
SELECT
    COALESCE(
        vos.ship_mode,
        'Unknown'
...
-- Comprobamos que la agrupación por modo de envío conserve todos los pedidos y líneas del modelo.
SELECT
...
-- Verificamos que los totales comerciales acumulados por modalidad coincidan con la vista por pedido.
SELECT
