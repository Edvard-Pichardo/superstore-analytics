/*
Archivo      : 08_vw_segment_performance
...
-- Cada fila representa un segmento e incluye pedidos, clientes, ventas, rentabilidad, logística y cobertura de información.
...
-- Comparamos los segmentos del catálogo con las filas generadas en la vista.
SELECT
...
-- Verificamos que ningún segmento del catálogo haya quedado fuera de la vista analítica.
SELECT
    s.segment_id,
    s.segment_name
FROM segments AS s
...
-- Comprobamos que la agrupación por segmento conserve todos los pedidos y líneas del modelo.
SELECT
...
-- Verificamos que los totales comerciales acumulados por segmento coincidan con la vista por pedido.
SELECT
