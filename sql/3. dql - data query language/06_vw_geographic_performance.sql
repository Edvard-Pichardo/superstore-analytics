...
-- Cada fila representa una combinación única de región, país, estado y ciudad. Los diferentes códigos postales
...
-- Verificamos que exista una fila por cada combinación única de región, país, estado y ciudad.
SELECT
    (
        SELECT COUNT(*)
        FROM
...
-- Verificamos que ninguna ciudad presente en los pedidos haya quedado fuera de la vista geográfica. (Debe devolver cero filas)
SELECT
    vos.region,
    vos.country,
    vos.state,
...
-- Comprobamos que la agrupación geográfica conserve todos los pedidos y líneas de venta.
SELECT
...
-- Verificamos que los totales comerciales de la vista geográfica coincidan con la vista por pedido.
SELECT
