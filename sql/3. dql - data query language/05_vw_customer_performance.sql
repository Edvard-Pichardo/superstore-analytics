/*
Archivo      : 05_vw_customer_performance
...
-- Cada fila representa un cliente e integra sus pedidos, ventas, cantidades, beneficios y cobertura de datos.
...
-- Comparamos los clientes del modelo con los clientes incluidos en la vista.
SELECT
...
-- Verificamos que ningún cliente del modelo haya quedado fuera de la vista analítica. (Debe devolver cero filas)
SELECT
    c.customer_id,
    c.customer_name
FROM customers AS c
...
-- Comprobamos que la agrupación por cliente conserve todos los pedidos y líneas de venta.
SELECT
...
-- Verificamos que las métricas acumuladas por cliente coincidan con los totales de la vista por pedido.
SELECT
