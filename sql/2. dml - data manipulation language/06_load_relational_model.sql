/*
Archivo: 06_load_relational_model
...
-- Vamos a cargar los datos a cada una de las tablas del modelo relacional que hicimos anteriormente. 
-- Recordemos que las tablas deben llevar un orden debido al modelo relacional.

-- Tabla segments
-- Insertamos los segmentos distintos que todavía no existen en la tabla de catálogo.
...
-- Revisamos que los datos hayan sido cargados correctamente. 
SELECT * FROM segments;


-- Tabla ship_modes
-- Los valores NULL no se incluyen en el catálogo porque representan modos de envío desconocidos.
...
-- Ahora vamos a cargar las tablas que tienen claves foráneas. 

-- Tabla sub_categories
-- Insertamos cada combinación única de categoría y subcategoría.
-- El JOIN permite sustituir el nombre de la categoría por su identificador interno category_id.
-- Nota: Aquí se debe tener cuidado porque en clean_sales aparece la columna subcategory, mientras que en stg_sales
-- aparece la columna sub_category. Fue un error al momento de crear la tabla, aunque no es importante si se mantiene esa distinción. 
...
WHERE cs.subcategory IS NOT NULL
  AND NOT EXISTS
  (
      SELECT 1
      FROM sub_categories AS sc
      WHERE sc.category_id = c.category_id
        AND sc.sub_category_name = cs.subcategory
  )
ORDER BY
    c.category_id,
    cs.subcategory;
...
-- haya quedado sin representación en el modelo relacional.
SELECT
    cs.category,
    cs.subcategory
FROM clean_sales AS cs
...
-- Devolvió cero por lo que no hay ninguna combinación que haya quedado fuera del modelo relacional. 


-- Tabla locations
-- Insertamos las ubicaciones únicas presentes en clean_sales.
-- El JOIN permite sustituir el nombre de la región por su identificador interno region_id.
...
-- Visualizamos una muestra de las ubicaciones junto con el nombre de su región.
SELECT
    l.location_id,
    l.country,
    l.state,
...
-- haya quedado fuera del modelo relacional.
SELECT
    cs.country,
    cs.state,
    cs.city,
...
-- Devolvió cero, por lo que ninguna ubicación se quedó fuera del modelo relacional. 


-- Tabla customers
-- Insertamos los clientes únicos presentes en clean_sales.
-- El JOIN sustituye el nombre textual del segmento por su identificador interno segment_id.
...
-- Visualizamos una muestra de clientes junto con el nombre de su segmento.
SELECT
    c.customer_id,
    c.customer_name,
    s.segment_name
...
-- tengan correspondencia en la tabla customers.
SELECT
    cs.customer_id,
    cs.customer_name,
    cs.segment,
...
-- Nuevamente, devolvió cero, por lo que todos los clientes tienen presencia dentro del modelo relacional. 

-- Aquí también podemos comparar la cantidad de clientes únicos del conjunto limpio con la cantidad cargada en el modelo relacional.
SELECT
    (
...
-- Tabla products
-- Insertamos cada combinación única de identificador original y nombre de producto.
-- Se utilizan categories y sub_categories para recuperar el sub_category_id correspondiente a cada producto.
...
-- Visualizamos una muestra de productos junto con su subcategoría y categoría.
SELECT
    p.product_key,
    p.source_product_id,
    p.product_name,
...
-- presentes en clean_sales tengan correspondencia en products.
SELECT
    cs.product_id,
    cs.product_name,
    cs.category,
...
WHERE p.product_key IS NULL
   OR sc.sub_category_name <> cs.subcategory
...
-- Devolvió cero, lo que significa que todas las combinaciones de nombre y código están presentes en el modelo relacional.

-- Comparamos los productos únicos del conjunto limpio con los productos cargados en el modelo relacional.
SELECT
    (
...
-- Un mismo identificador original puede representar más de un pedido cuando aparece asociado con ubicaciones distintas.
...
-- Comparamos la cantidad de pedidos lógicos del conjunto limpio con la cantidad almacenada en el modelo relacional.
SELECT
    (
        SELECT COUNT(*)
        FROM
...
-- Visualizamos una muestra con los datos descriptivos provenientes de las tablas relacionadas.
SELECT
    o.order_key,
    o.source_order_id,
    o.order_date,
...
-- Perfecto. Ahora nos queda cargar los datos a la tabla order_details. 
-- Insertamos cada línea de venta presente en clean_sales.
-- Para localizar el pedido se utilizan su identificador original, cliente, ubicación, fechas y modo de envío.
...
-- Comparamos la cantidad de filas del conjunto limpio con la cantidad de detalles del modelo relacional.
SELECT
...
-- Comparamos los totales del conjunto limpio con los almacenados en el modelo relacional.
SELECT
