/*
Archivo: 07_validate_relational_model
...
-- Primero, vamos a realizar una cnciliación de cantidades.
-- Comparando las entidades únicas presentes en clean_sales con las filas almacenadas en cada tabla relacional.
WITH controles AS
(
    -- Segments
    SELECT
...
-- Perfecto. La cantidad de registros de cada zona de control es correcta. 


-- Ahora, comprobamos que todas las claves foráneas tengan correspondencia en sus respectivas tablas principales.
WITH controles AS
(
...
-- Bien, no tenemos ningún registro inválido en ninguna de las zonas de control de nuestro modelo relacional.
-- Esto significa que todas las claves foráneas están apuntando o tienen correspondencia hacia sus propias tablas principales. 


-- Ahora, busquemos grupos duplicados en las columnas que identifican de manera única a cada entidad del modelo relacional.
WITH controles AS
(
...
-- No hay ningún grupo duplicado, lo que quiere decir que cada dato se identifica de manera única y coherente en el modelo relacional.


-- Esto ya lo hicimos antes (más o menos), pero es importante revisar que esté correcto. 
-- Debemos comprobar las reglas principales del negocio, es decir, comprobamos fechas, métricas comerciales e identificadores.
-- Los valores NULL de sales, quantity y profit pueden ser válidos. 
WITH controles AS
(
    ...
    UNION ALL
    -- El descuento es obligatorio y debe encontrarse dentro del intervalo de 0 a 1.
    SELECT
...
-- Bien, las reglas principales que establecimos desde un inicio se cumplen a la perfección. No hay ninguna que se incumpla. 


-- Realicemos un diágnostico para conocer los valores faltantes de algunas columnas. 
SELECT
...
-- Tenemos ventas desconocidas, cantidades desconocidas, beneficios desconocidos, operaciones con perdida y operaciones sin beneficio.
-- Esto no necesariamente son errores o una flata de integridad en la base de datos. 

-- Los pedidos con ship_mode_id NULL se conservaron porque no existía evidencia suficiente para completar el dato.
SELECT
...
-- Tenemos 274 registros con envío desconocido. 


-- Comparemos las métricas de clean_sales con las almacenadas en order_details.
-- Además de los totales, verificamos que los valores NULL se hayan conservado y no se hayan convertido en cero.
...
-- Todos los datos se cargaron bien al modelo relacional, no hay problema con ninguno de ellos.