...
Descripción  : Valida de forma integral la configuración, funcionamiento e integridad del sistema de
...
-- Verificamos que existan exactamente las ocho columnas definidas para auditoría.
SELECT
...
-- Verificamos los índices definidos para las consultas de auditoría.
SELECT
    index_name,
    non_unique,
    GROUP_CONCAT(
...
-- Verificamos que exista la restricción de coherencia entre acción y payload JSON.
SELECT
    tc.constraint_name,
    tc.constraint_type,
    cc.check_clause
...
-- Comprobamos que no existan registros cuya estructura contradiga el tipo de acción.
SELECT
...
-- Recuperamos explícitamente la clave del pedido temporal.
SET @test_order_key =
(
    SELECT order_key
    FROM orders
...
-- Verificamos que se hayan generado exactamente tres eventos de auditoría.
SELECT
...
-- Comprobamos que exista exactamente un evento de cada tipo.
SELECT
    action_type,
...
-- Inspeccionamos la secuencia completa registrada por los triggers.
SELECT
    audit_id,
    action_type,
    changed_at,
...
-- Validamos específicamente el cambio realizado sobre ship_date.
SELECT
    JSON_UNQUOTE(
        JSON_EXTRACT(
            old_data,
...
-- Validamos la coherencia de los payloads para los tres eventos de la prueba.
SELECT
    SUM(
        CASE
...
-- Confirmamos que ni el pedido temporal ni sus eventos de auditoría persistieron.
SELECT
    (
        SELECT COUNT(*)
        FROM orders
...
-- Recuperamos explícitamente la clave de la línea temporal.
...
-- Modificamos las ventas y el beneficio para generar un UPDATE real.
...
-- Validamos los cambios registrados sobre ventas y beneficio.
SELECT
    CAST(
        JSON_UNQUOTE(
            JSON_EXTRACT(
...
-- Confirmamos que ni la línea temporal ni sus eventos de auditoría persistieron.
SELECT
    (
        SELECT COUNT(*)
        FROM order_details
...
-- Contamos las auditorías UPDATE existentes para el pedido seleccionado.
...
-- Ejecutamos un UPDATE que no modifica realmente ningún valor del pedido.
UPDATE orders
...
-- Contamos las auditorías UPDATE existentes para la línea de pedido seleccionada.
...
-- Ejecutamos un UPDATE que no modifica realmente ningún valor de la línea.
UPDATE order_details
SET sales = sales
WHERE order_detail_key = @test_order_detail_key;
...
-- Validamos el comportamiento del trigger de order_details.
SELECT
...
-- Verificamos que no existan tipos de acción diferentes de los definidos por el sistema.
SELECT
...
-- Verificamos nuevamente la coherencia global entre el tipo de acción y los payloads JSON.
SELECT
...
-- Verificamos que todas las claves auditadas tengan un valor identificable.
SELECT
...
-- Verificamos que todos los eventos tengan información sobre la conexión responsable.
SELECT
...
-- Generamos un resumen final de las validaciones críticas del sistema.
